...
# 1. Model 1: Spatial Suppression congeneric -----------------------------------

congenericSupp2 <- 'gSupp2 =~ S1mean + S2mean + S3mean + S4mean
                    S3mean ~~ a1*S3mean; a1>0'
...
semPaths(congenericSupp2Fit,what= "std",style= "lisrel",layout= "tree", structural = F,cut= .60,residScale= 7,
...
# BIS congeneric ----------------------------------------------------------
congenericBIS <- 'g =~ zNumeric + zFigural + zVerbal'

congenericBISFit <- sem(congenericBIS,
                     data      = res,        #Datensatz mit den in sem.model verwendeten Variablen
                     estimator = "MLM",      #Schätzmethode ist die robuste Variante von ML
                     mimic     = "Mplus")    #man kann den Output von Mplus (default ist EQS)
...
summary(congenericBISFit, 
        fit.measures  = T,                #Fit Indizes
        standardized  = T,                #Parameter werden zusätzlich standardisiert
        rsquare       = T)                #Erklärte Varianz der manifesten Variablen wird ausgegeben
parameterEstimates(congenericBISFit,
                   standardized  =T)     #parameter estimates werden standardisiert aufgeführt

semPaths(congenericBISFit,what= "std",style= "lisrel",layout= "tree", structural = F,cut= .60,residScale= 7,
         sizeMan= 7,sizeLat= 5,intercepts= F,optimizeLatRes = T,exoVar= T,nCharNodes= 0,
         asize= 1.5,esize= 1,edge.color= 1,edge.label.cex = .5,cardinal= F,nDigits= 3,rotation= 2)

predict(congenericBISFit)
corr.test(res[c("S1mean", "S2mean", "S3mean", "S4mean")], predict(congenericBISFit))

# 2. Model 2: Spatial Suppression und g ----------------------------------------------------
