...
# 6. Correct for the hard coded stimulus duration limit of 1000 ms -------------
names(results[2:25])
results[2:25][results[2:25]>400]  <- 400              #replace all thresholds > 400 with 400

# 7. Drop subjects who hit the built in stimulus duration limit more than once ----
...
# 8. Create actual stimulus duration -------------------------------------------
...
# 10. Vector of lowest/highest thresholds ---------------------------------------
# Get a vector of the smallest and highest thresholds
# per stimulus size (s1, s2, s3, s4).
# apply function: Argument 1 = indicates rows 
names(results[52:57])
names(results[58:63])
names(results[64:69])
names(results[70:75])

subjectMins1 <- apply(results[52:57],1,min)
subjectMaxs1 <- apply(results[52:57],1,max)

subjectMins2 <- apply(results[58:63],1,min)
subjectMaxs2 <- apply(results[58:63],1,max)

subjectMins3 <- apply(results[64:69],1,min)
subjectMaxs3 <- apply(results[64:69],1,max)

subjectMins4 <- apply(results[70:75],1,min)
subjectMaxs4 <- apply(results[70:75],1,max)

# 11. Create log10 mean threshold and sd of thresholds -------------------------
# 11.1 Compute mean threshold without the smallest and highest estimation ------
# and create new variable /w prefix log10mean
results$S1log10mean  <- round((rowSums(results[52:57]) - subjectMins1 - subjectMaxs1) / 4, digits = 4)
results$S2log10mean  <- round((rowSums(results[58:63]) - subjectMins2 - subjectMaxs2) / 4, digits = 4)
results$S3log10mean  <- round((rowSums(results[64:69]) - subjectMins3 - subjectMaxs3) / 4, digits = 4)
results$S4log10mean  <- round((rowSums(results[70:75]) - subjectMins4 - subjectMaxs4) / 4, digits = 4)

# 11.2 Compute sd of thresholds ------------------------------------------------
sdCustom <- function(x){
            x <- x[x!=min(x) & x!=max(x)]
            return(sd(x))
            }
...
# 12. Create mean threshold and sd (used mainly for plotting) ------------------
results$S1mean <- round(10 ^ results$S1log10mean, digits = 0)
results$S2mean <- round(10 ^ results$S2log10mean, digits = 0)
results$S3mean <- round(as.numeric(10 ^ results$S3log10mean, digits = 0))
...
# 13. Compute 'SUPPRESSION INDEX 2' ----------------------------------------------
...
# resultsSupp2 <- subset(resultsSupp2, !subject %in% c(xxxx))
...
print(cov(resultsSupp2[4:7]), digits = 2)
print(cov(resultsSupp2[12:15]), digits = 0)
nrow(resultsSupp2)
...
# Plot thresholds for each size -------------------------------------------
par(mfrow=c(1,4))
plot(sample(resultsSupp2$S1mean),
...
axis(side = 2, at = seq(0,400,100), las=2)
abline(h = mean(resultsSupp2$S1mean), col = "blue", lwd = 2)
# abline(h = 1500, col = "red", lwd = 2 )

plot(sample(resultsSupp2$S2mean),
...
axis(side = 2, at = seq(0,400,100), las=2)
abline(h = mean(resultsSupp2$S2mean), col = "blue", lwd = 2)

plot(sample(resultsSupp2$S3mean),
...
axis(side = 2, at = seq(0,400,100), las=2)
abline(h = mean(resultsSupp2$S3mean), col = "blue", lwd = 2)

plot(sample(resultsSupp2$S4mean),
...
axis(side = 2, at = seq(0,400,100), las=2)
abline(h = mean(resultsSupp2$S4mean), col = "blue", lwd = 2)
...
plot(resultsSupp2$S1mean, resultsSupp2$S4mean,
     ...
     type  = "p",
     cex   = .8,
     col = ifelse(resultsSupp2$S1mean>resultsSupp2$S4mean, "red", ifelse(resultsSupp2$S1mean>200, "mediumorchid2", "black")),
     pch = 19,
...
# 10. Melnick et al. (2013) ------------------------------------
# 10.1 raw SIs of Melnick et al.'s study 2
...
# 10.2 Dotchart of Melnick et al.'s values
# dotchart(dujeSI,
#          cex = .7,
#          xlab = "Suppression Index [log10]",
...
# 10.3 Compare Melnick et al.'s & our densities --------------------------------
# par(mfrow=c(1,2))
...
# 10.5 Histogram plot of Melnick et al. (2013) data ----------------------------
par(mfrow=c(2,1))
hist(dujeSI,
     breaks = 80000,
...
# 10.6 Histogram plot of bern data ----------------------------
hist(resultsSupp2$SI2,
     breaks = 80000,
     main = paste("Bern data, N = ", nrow(resultsSupp2), sep = ""),
     ylim = c(0,4),
     xlim = c(-.2,1),
     xaxt = "n",
     yaxt = "n",
