...
###   Duration Discrimination with EMPTY intervals (DDE)
###
###   Exercise: 5 trials
###   Test:
...
setwd("/Users/philippthomas/Documents/Uni Bern/PDD/Dissertation/Daten/Project/data/base/DDE")
...
list9 <- lapply(list8, function(x){
                       ...
                       x$`list$DDEdl`        <- (abs(x[1,5]) + abs(x[1,6]))/2
                       x})
...
colnames(result)  <- c("t1a","t2a","t3a","t4a","t5a","t6a","t7a","t8a","t9a","t10a",
                       ...
                       "t31b","t32b",
                       "meanDelta25","meanDelta75","DDEdl")

# for plotting the curves change to "as.numeric(subst.....)
